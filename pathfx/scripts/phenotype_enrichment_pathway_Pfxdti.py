...
dtd = pickle.load(open('../rscs/Pfxdti_dint.pkl','rb'))
db = pickle.load(open('../rscs/Pfxdti_dbid2name.pkl','rb')) # Drugbank ID to name
...
def do_network(tlist,aname,outdir,dname,doCluster):
	...
	cmd = 'python ../scripts/get_network_associations_Pfxdti.py -f %s -a %s -d %s -n %s' % (froot,dname,outdir,len(tlist))
	os.system(cmd)
...
def main():
	...
	if len(t_in_n) > 0:
		print('')
		print('number of targets within the interactome: ', len(t_in_n))
